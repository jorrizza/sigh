# Sigh disk collector collecting Linux disk write latency

Sigh::Collector.collects do
  type 'disk'
  name 'sda_write_latency'
  unit 'ms'
  ...
  measure do
    ...
    2.times do
      unless first_io
        ...
        first_io = str.strip.split[4].to_f
        first_ticks = str.strip.split[7].to_f
        sleep 1
      else
        str = File.read("/sys/block/#{disk}/stat")
        second_io = str.strip.split[4].to_f
        second_ticks = str.strip.split[7].to_f
      end
    end
